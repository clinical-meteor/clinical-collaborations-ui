...
function memberBecause(id) {
    var col = Collaborations.findOne({_id: id});
    ...
    function () {
        Template.collaborationGrid.helpers({
            collaboration: function () {
                return Collaborations.find({}, {sort: {order: 1, name: 1}});
            }
        ...
        function prettyList(cols) {
           ...
           cols.map(function(col,i) {
               if (col.indexOf("@") < 0) {
                   var c = Collaborations.findOne({"_id":col});
                   if (c)
                       answerSet[c.name] = 1;
                   else
                       answerSet[col] = 1;
        ...
        Template.collaborationGridElement.helpers({
            ...
            requiresApproval: function () {
                console.log("requiresApproval", this, Collaborations.findOne({_id: this._id}));
                var col = Collaborations.findOne({_id: this._id});
        ...
        Template.collaborationGridElement.events({
            ...
            'click .collaborationRemove': function(tmpl, vent) {
                if (confirm("Are you sure you want to remove " + this.name + "?"))
                  Collaborations.remove({_id: this._id})
             }
        });
    }
